...
test('POST /usuario - Cadastro de usuarios deve retornar status 400 ao enviar um corpo de requisição inválido', async () => {
...
test('POST /usuario - Cadastro de usuarios deve retornar status 409 ao enviar um email que já existe no banco de dados', async () => {
  const response = await request(server)
    .post('/usuario')
...
test('GET /usuario sem token retorna status 401', async () => {
  const response = await request(server)
    .post('/login')
    .set('Authorization', `Bearer `) // Defina o cabeçalho Authorization com o token
  expect(response.status).toBe(401);
});
...
test('GET /usuario com ativo retorna status 200', async () => {
  const response = await request(server)
    .post('/login')
