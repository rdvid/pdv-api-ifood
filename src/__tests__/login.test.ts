...
test('POST /cliente - Cadastro de clientes deve retornar status 409 ao enviar um corpo de requisição inválido', async () => {
  const response = await request(server)
    .post('/cliente')
...
test('PUT /cliente/:id - Alteração do cadastro de clientes deve retornar status 201 ao enviar um corpo de requisição inválido', async () => {
  const response = await request(server)
    .post('/cliente')
